...
import br.com.demo.adapter.outbound.address.nosql.mongo.util.MongoAddressModelUtil;
...
public class MongoAddressPatchOutboundService implements AddressPatchPortOubound {
	...
	private final MongoAddressModelUtil util = MongoAddressModelUtil.getInstance();
	...
	public void execute(AddressCoreDTO addressFoundToPatch, AddressCoreDTO patchAddressCoreDTO) {
		...
		util.patch(addressFoundToPatch, patchAddressCoreDTO);

		repository.save(mapper.from(addressFoundToPatch));

		log.info("address updated patch with success ");
